...
### Added
- Added Laravel Service Provider (`StatumServiceProvider`) with publishable config support.
- Added Laravel package auto-discovery mapping in `composer.json`.
- Restructured `README.md` to add Getting Started details and collapsible wire-level JSON request/response schema specifications.

### Changed
- Replaced the example Sender ID `'STHERESACWA'` with `'STATUM'` in the documentation.
...
### Fixed
- Fixed PHP 8.1 parse error in DTOs by replacing `readonly class` (introduced in PHP 8.2) with standard classes using individual `readonly` properties.
